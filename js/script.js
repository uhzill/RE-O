...
$(document).ready(function(){
    ...
    // 물결
    $(".gradientBg").ripples({
        resolution: 250, // 값이 클수록 잔물결 느리게 전파
        perturbance: .001, // 잔물경 굴절 강도. (0은 굴절 없음)
    });
    ...
    function Obj(x, y, z) {this.set(x, y, z);}
    ...
    function draw() {
        ...
        for (var i in arr) {
            ...
            dth = ((p.rp.z / dep) + 1);
            d.fillStyle = p.col;
            d.fillRect(w + p.rp.x, h + p.rp.y, rnd(dth/0.8, dth/2),  dth/0.9);
        }
    }
    ...
    function run() {
        d.clearRect(0, 0, w, h);
        var g_ = d.createLinearGradient(c.width + c.width,
